...
export function evaluatePolicy(claim: Partial<ExpenseClaim>): PolicyResult {
  ...
  if (!claim.flags?.businessPurpose) {
    result.blocks.push("Confirm business purpose.");
  }
  if (!claim.flags?.policyConfirmed) {
    result.blocks.push("Confirm policy compliance.");
  }
  ...
  const hasHCP = attendees.some((a: Attendee) => a.role === "HCP" && (a.name ?? "").trim() !== "");
  if (!hasHCP) {
...
export function canSubmit(claim: Partial<ExpenseClaim>): boolean {
  const policy = evaluatePolicy(claim);
  return policy.blocks.length === 0;
}
