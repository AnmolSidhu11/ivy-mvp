...
interface InvocationContext {
  ...
  bindingData: { claimId?: string; path?: string };
  bindings: {
    triggerBlob?: Buffer | string;
    silverBlob?: unknown;
    goldBlob?: unknown;
    auditBlob?: unknown;
  };
...
function parseClaim(triggerBlob: Buffer | string): ExpenseClaim | null {
  try {
    const raw = typeof triggerBlob === "string" ? triggerBlob : triggerBlob.toString("utf8");
    return JSON.parse(raw) as ExpenseClaim;
  } catch {
    return null;
  }
}
 ...
 */
function enrichClaim(claim: ExpenseClaim): ExpenseClaim {
...
async function blobTriggerPolicyEnricher(
  ...
  (context.bindings as Record<string, unknown>).silverBlob = JSON.stringify(silverPayload, null, 2);

  // Gold: current state for dashboard
  (context.bindings as Record<string, unknown>).goldBlob = JSON.stringify(enriched, null, 2);
  ...
  (context.bindings as Record<string, unknown>).auditBlob = JSON.stringify(auditEntries, null, 2);

  context.log("Policy enricher completed for claimId=%s, status=%s", claimId, enriched.status);
}

// Azure Functions Node v3: runtime expects module.exports = function(context, triggerBlob)
export = blobTriggerPolicyEnricher;
