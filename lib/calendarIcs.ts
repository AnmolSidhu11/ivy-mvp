/**
 * Build ICS (iCalendar) string with multiple VEVENTs.
 * DTSTAMP in UTC (Z). DTSTART/DTEND as floating local time (no Z).
 * No Math.random in render path; UID is deterministic when provided.
 */
...
function formatDtstampUtc(d: Date): string {
...
export function buildIcsCalendar(events: IcsEventInput[]): string {
  ...
  let ics = "BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//Sanofi Concierge//EN\r\n";
  for (const e of events) {
    const uid = e.uid ?? `evt-${e.start.getTime()}-${e.end.getTime()}-${(e.summary ?? "").slice(0, 20)}`;
