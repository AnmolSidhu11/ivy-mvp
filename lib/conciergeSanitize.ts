const PHI_MARKERS = ["patient", "dob", "date of birth", "mrn"];
...
export function checkForPhi(input: string): string | null {
  ...
  if (PHI_MARKERS.some((m) => lower.includes(m))) {
    return "Content appears to include patient identifiers (e.g., patient, DOB, MRN). Please remove PHI and try again.";
  }
  return null;
}
